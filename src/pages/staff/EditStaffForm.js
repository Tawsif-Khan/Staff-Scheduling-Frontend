...
export default function EditStaffForm() {
  ...
  async function getStaffData() {
    try {
      const response = await get(`api/get/employee/${id}`);
      console.log(response);
      setName(response?.data.name);
      setSid(response?.data.sid);
      setPhone(response?.data.phone);
      setGender(response?.data.gender);
      setPrefMorning(response?.data.pref_morning);
      setPrefAfternoon(response?.data.pref_afternoon);
      setPrefNight(response?.data.pref_night);
      let weekends = [...weekendCheck];
      weekends[parseInt(response?.data.pref_weekend_1) - 1] = false;
      weekends[parseInt(response?.data.pref_weekend_2) - 1] = false;
      setWeekendCheck(weekends);
      setDisableWeekendCheck(false);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await post(`api/data/${id}`, payload);
      console.log(response);
    } catch (error) {
      console.error("Error creating app:", error);
    } finally {
